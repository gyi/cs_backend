...
$(function(){
	...
	$(".appbtn").live("click", function(){
		...
		art.dialog.open("../manage/manage.action?id="+appid, {id : id,title: title, width:1280, height:782, narrow:false,
			close: function () {
				auiCloseClick(id);
		    }	
		});
		...
		if($("#L" + id).find(".aui_narrow").size()==0){
			...
			if(auiClose.length==0){
				...
				$($(".aui_outer[id*=LartDialog]").find(".aui_close")[0]).attr("id", "C"+id);
				$($(".aui_outer[id*=LartDialog]").find(".aui_close")[0]).before('<a class="aui_narrow" href="javascript:;" id=N' + id + '>\u005f</a>') ;
				if($(".aui_outer[id*=LartDialog]").find(".aui_close").size()>0){
					$($(".aui_outer[id*=LartDialog]").find(".aui_close")[0]).before('<a class="aui_narrow" href="javascript:;" id=N' + id + '>\u005f</a>') ;
					$($(".aui_outer[id*=LartDialog]").find(".aui_title")[0]).before('<img class="aui_narrow_icon" src="'+img+'"/>') ;
				}else if($(".aui_outer[id*=LTips]").find(".aui_close")){
					$($(".aui_outer[id*=LTips]").find(".aui_close")[0]).before('<a class="aui_narrow" href="javascript:;" id=N' + id + '>\u005f</a>') ;
					$($(".aui_outer[id*=LTips]").find(".aui_title")[0]).before('<img class="aui_narrow_icon" src="'+img+'"/>') ;
				}if($(".aui_outer[id*=LConfirm]").find(".aui_close")){
					$($(".aui_outer[id*=LConfirm]").find(".aui_close")[0]).before('<a class="aui_narrow" href="javascript:;" id=N' + id + '>\u005f</a>') ;
				}
			}else{
				$($("#L" + id).find(".aui_close")[0]).attr("id", "C"+id);
				$($("#L" + id).find(".aui_close")[0]).before('<a class="aui_narrow" href="javascript:;" id=N' + id + '>\u005f</a>') ;
				$($("#L" + id).find(".aui_title")[0]).before('<img class="aui_narrow_icon" src="'+img+'"/>') ;
			}
		}
		taskbar(id, title);
...
function taskbar(id, title) {
	var img = $("#"+id).children("div").children("img").attr("src");
	var hidden_temp = '<div id="task-content-inner" class="task-inner" hidenId="'+id+'" style="width: 114px; margin-left: 0px; margin-right: 0px;">'
			...
			+ '<div class="task-item-icon"><img src="'+img+'" alt="'+title+'"/></div>'
